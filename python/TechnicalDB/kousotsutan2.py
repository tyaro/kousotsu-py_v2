...
def Judgement(df,btcRSI):
    ...
    df.loc[(df[SP_] > 0) & (df['RSI14_4H'] < btcRSI-10),SP_] = 2  
    # ロングの場合RSI14が40を上回っていたら星追加
    df.loc[(df[LP_] > 0) & (df['RSI14_4H'] > btcRSI+10),LP_] = 2  
    ...
    # EMA200及びEMA100を満たしているときにEMA50の条件を確認
    df.loc[(df[SP_] > 3) & (df['DREMA50'] > 0),SP_] = df[SP_] + 1
    df.loc[(df[LP_] > 3) & (df['DREMA50'] < 0),LP_] = df[LP_] + 1
...
def getBtcRSI(client):
    value = client.get('RSI_INFO_BTCUSDT')
    d = json.loads(value)
    return d['RSI14_6H']['VALUE']
